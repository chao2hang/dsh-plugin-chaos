...
# The build preserves the face order from package.json: bootstrap the typert
# contracts first, then compile and bundle the Host face before compiling and
# bundling the Client face. The bootstrap writes arch-independent declaration
# files, so the host Node runs it once before Wine takes over.
# Both statuses are captured so one failure cannot hide the other's result.
build_gate() {
  node --import tsx/esm scripts/bootstrap-typert.ts || return $?

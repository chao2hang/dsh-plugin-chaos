...
- The root `tsdown.config.ts` keeps the upstream Host entry `['lib/types/{index,invariant,startup}.js']`; the re-baseline had flattened it to `''`, which silently stopped the workspace tsdown pass from bundling every package that lacks a package-local config. 168 workspace packages (the vendored framework and its plugin family included) then had an exports map pointing at a `lib/index.js` nothing produced: the web build could not resolve `@deepseek-ai/cordis`, the built CLI could not resolve `@deepseek-ai/cordis-plugin-group` or `@deepseek-ai/dsh-home-paths`, and the pack rehearsal failed publint. Restoring the entry bundles each such package from the JavaScript tsc emitted under `lib/types`.

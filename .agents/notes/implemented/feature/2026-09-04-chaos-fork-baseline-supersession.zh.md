...
推送后远端旧基线提交从 `chaos/master` 不可达（在被回收前仍可按哈希取回）。找回的 `chaos-sandbox-guidance` 当天即被所有者决定移除（见 chaos-unused-removal 笔记），从未激活。调和依赖人工对比两个分支尖端；未来从过期树上切出的迁移需要同样的审计——取代前先与远端对比 chaos 插件目录。

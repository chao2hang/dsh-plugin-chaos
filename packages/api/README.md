...
| [`session-controller/`](session-controller/README.md) | Owns Session commands, history streams, live control state, Agent/Session identity policy, and all-history usage reporting. | `ctx.sessionController`, `ctx.usageReportController` / `ctx.remote.session`, `ctx.remote['usage-report']` |

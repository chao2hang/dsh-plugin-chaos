...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({ preference: DEFAULT_PREFERENCE, uiFontSize: 14, codeFontSize: 12 })
    await ctx.settings.update(ns, { preference: 'dark', uiFontSize: 15, codeFontSize: 13 })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', uiFontSize: 15, codeFontSize: 13 })

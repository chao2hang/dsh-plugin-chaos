...
describe('createAppearanceRowStore', () => {
  it('init shape: system preference with revision at -1', () => {
    ...
    expect(store.getSnapshot()).toEqual({ preference: 'system', uiFontSize: 14, codeFontSize: 12, revision: -1 })
  })

  it('sync mirrors the preference and advances the revision', () => {
    const store = createAppearanceRowStore().create()
    store.actions.sync('dark', 14, 12, 0)
    expect(store.getSnapshot()).toEqual({ preference: 'dark', uiFontSize: 14, codeFontSize: 12, revision: 0 })
    store.actions.sync('light', 15, 13, 2)
  ...
  it('revision guard drops stale and duplicate writes', () => {
    const store = createAppearanceRowStore().create()
    store.actions.sync('dark', 14, 12, 3)
    store.actions.sync('system', 16, 14, 2)
    store.actions.sync('system', 16, 14, 3)

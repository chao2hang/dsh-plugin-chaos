...
describe('UI renderer plugin', () => {
  ...
  it('hydrates the boot page before switching to the assembled application', async () => {
    ...
    slots.register({ name: 'root' }, () => <div data-testid="root-probe" />)
    const el = container()
    el.innerHTML = '<div class="boot" data-dsh-boot=""><div><div>启</div><div>DEEPSEEK HARNESS</div><div>正在进入</div><div class="spinner" data-dsh-boot-spinner="" style="--dsh-boot-arc: 180deg"></div><div role="status">正在准备工作区</div></div></div>'

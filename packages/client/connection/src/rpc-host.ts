...
import { bridge, isAuthenticatedApiRequest, type FetchHandler } from './http-bridge.ts'
  ...
  private register(
    ...
    const route: WebRoute = {
      ...
      handler: async (req, res) => {
        if (!isTrustedApiRequest(req, trustedHosts) && !this.ctx.webServer.isAuthenticated(req)) {
          res.writeHead(403)
          res.end('forbidden')
          return
        }

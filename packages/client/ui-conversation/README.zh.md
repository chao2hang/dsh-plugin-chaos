...
Statistics 标签页将浏览器的 IANA 时区传给宿主 `usage-report.read` 端点，按查看者的日历日展示所有可读取持久化历史中已完成请求的汇总和模型路由。其 30 天本地日历图包含每天，将零用量日期显示为中性灰色，并按持久化 provider/model 路由堆叠当天的真实 Token 用量；未匹配的余量会明确标为未归因。图表只显示一次日期范围；悬停、点击或使用键盘选择日期即可查看当天的 Token、请求数和模型分段。标签页重新挂载时，会立即显示该连接端点与时区最近一次成功的报表，并在后台刷新。页面在前台时每 30 秒重新读取持久数据，切回页面和跨过下一个本地零点时立即刷新，并显示最近成功的本地更新时间和手动刷新操作。首次读取报表期间，带标签的骨架屏会保持汇总和图表的版式。它绝不会把会话汇总值分配到会话更新时间：每个桶使用其持久化 `assistant/message.usage` 事件时间。读取失败会显示重试操作，且不显示成本，因为请求时的实际价格没有持久化。
...
Chat 业务行是彼此独立的注册表贡献，不是封闭的内建联合。Client 插件通过 declaration merging 增加类型化 `ChatNodeDataMap` key，在 `ctx.conversationEvents` 上注册 `ConversationNodeDefinition`，再向 `conversation.chat.node` 注册匹配的 keyed renderer；它无须修改会话 fold 或中央 renderer switch。稳定事件 id、append/prepend 回放、Location data 与 renderer 约束见 [Conversation 子系统参考](../../../docs/subsystems/conversation.zh.md)。

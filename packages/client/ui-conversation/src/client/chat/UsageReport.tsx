...
/** Props supplied by the Statistics conversation tab. */
...
export type UsageReportProps = ConvViewProps & PropsLocale<'conversation'> & {
  usageReport?: {
    read: (payload: { timeZone: string }, signal?: AbortSignal) => Promise<{
      result: { ok: true; value: UsageReportData } | { ok: false; error: unknown }
    }>
  } | undefined
}
...
type UsageReportEndpoint = NonNullable<UsageReportProps['usageReport']>
...
const usageReportCache = new WeakMap<UsageReportEndpoint, Map<string, CachedUsageReport>>()

function cachedUsageReport(endpoint: UsageReportEndpoint | undefined, timeZone: string): CachedUsageReport | undefined {
  return endpoint === undefined ? undefined : usageReportCache.get(endpoint)?.get(timeZone)
}

function cacheUsageReport(endpoint: UsageReportEndpoint, timeZone: string, report: UsageReportData, updatedAt: number): void {
...
export function UsageReport({ usageReport, t }: UsageReportProps) {
  ...
  const cards = totals === null ? [] : [
    { label: t('report.totalTokens'), value: formatTokens(totalTokens(totals)) },
    { label: t('report.inputTokens'), value: formatTokens(totals.inputTokens + totals.cacheReadTokens + totals.cacheWriteTokens) },
    { label: t('report.outputTokens'), value: formatTokens(totals.outputTokens) },
      ...
      {state === 'ready' && report !== null && totals !== null && totals.requests > 0 && <>
        ...
        <section className={css.panel} aria-label={t('report.activity')}>
          ...
          <div className={css.activity} aria-label={t('report.activity')}>
            {trendDays.map(day => <span key={day.date} data-level={day.tokens === 0 ? 0 : Math.min(4, Math.ceil(day.tokens / peakTokens * 4))} data-zero={day.tokens === 0 || undefined} title={day.date + ': ' + formatTokens(day.tokens)} />)}
          </div>
        </section>
        <section className={css.panel} aria-label={t('report.trend')}>
          ...
          <div className={css.chart} role="group" aria-label={t('report.trend')}>
            {trendDays.map(day => <button
              ...
              aria-label={t('report.daySummary', { date: day.date, tokens: formatTokens(day.tokens), requests: day.requests })}
          ...
          {selectedDay !== undefined && <div className={css.dayDetail} aria-live="polite">
            ...
            <div><strong>{formatTokens(selectedDay.tokens)}</strong><span>{t('report.totalTokens')} · {selectedDay.requests} {t('report.requests')}</span></div>
            <div className={css.daySegments}>
              {selectedDay.tokens === 0
                ? <span className={css.noUsage}>{t('report.noUsage')}</span>
                : selectedDay.segments.map(segment => <span className={css.legendItem} key={segmentKey(segment)}><i style={{ backgroundColor: colors.get(segmentKey(segment)) }} />{segmentLabel(segment, t('report.unattributed'))} · {formatTokens(segment.tokens)}</span>)}
            </div>
          </div>}
        ...
        <section className={css.panel} aria-label={t('report.models')}>
          ...
          <div className={css.models}>
            {report.models.map(model => <div key={model.provider + '/' + model.model}><strong>{model.provider} / {model.model}</strong><span>{formatTokens(totalTokens(model))} · {model.requests} {t('report.requests')}</span></div>)}
            {report.unattributed.requests > 0 && <div><strong>{t('report.unattributed')}</strong><span>{formatTokens(totalTokens(report.unattributed))} · {report.unattributed.requests} {t('report.requests')}</span></div>}
          </div>
        </section>
      </>}
    </section>

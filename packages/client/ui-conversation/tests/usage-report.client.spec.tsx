...
const usageReport = { read: async () => ({ rpcId: 'usage' as never, result: { ok: true as const, value: report } }) }
const props = { sessionId: 'first' as never, useSession: (() => undefined) as never, useSessions: (() => undefined) as never, useWorkspaces: (() => undefined) as never, useProjection: (() => undefined) as never, useInput: (() => undefined) as never, inputActions: {} as never, t }
...
describe('UsageReport', () => {
  ...
  it('refreshes completed-request data every 30 seconds while visible', async () => {
    ...
    const live = { read: async (payload: { timeZone: string }) => {
      ...
      calls += 1
      return { rpcId: 'usage' as never, result: { ok: true as const, value: calls === 1 ? report : {
        ...report,
        days: [{
          ...report.days[0],
          inputTokens: 40,
  ...
  it('shows the prior report immediately while a remounted tab refreshes it', async () => {
    ...
    const endpoint = { read: () => {
      calls += 1
      return calls === 1
        ? Promise.resolve({ rpcId: 'usage' as never, result: { ok: true as const, value: report } })
        : refreshPending
    } }
  ...
  it('shows the empty state for a real report with no completed requests', async () => {
    ...
    const view = render(<UsageReport {...props} usageReport={{ read: async () => ({ rpcId: 'usage' as never, result: { ok: true as const, value: empty } }) }} />)
  ...
  it('shows a retry action after a transport or RPC failure', async () => {
    ...
    const failing = { read: async () => {
      calls += 1
      return { rpcId: 'usage' as never, result: calls === 1 ? { ok: false as const } : { ok: true as const, value: report } }
    } }

...
/** Drain the stubbed rAF queue in order: restore settlement runs per frame. */
function pumpFrames(frames: FrameRequestCallback[]): void {
  act(() => {
...
describe('ChatView', () => {
  ...
  it('settles an above-bottom restore after the remounted flow reflows', () => {
    ...
    document.body.appendChild(host)
    let anchorTop = 80
    // The rail schedules its active-turn frame before the restore settlement
    // frame, so the pump drains every pending frame, not just the first.

...
在移动视口中，明确的水平滑动会显示既有的 Session 管理控件，而不依赖只在悬停时出现的省略号：右滑显示重命名和分叉，左滑显示归档。归档会在持久化 registry 请求完成前立即隐藏该行；请求失败时恢复显示。垂直或过短的手势不改变行状态，滑动也不会打开 Session；空白的「新会话」占位行不提供这些控件。

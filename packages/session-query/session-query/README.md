...
- `projectSessions(sessionIds, project, signal?)` folds selected live-preferred full logs while each source is borrowed, returning ordered per-session settlements. Persisted inspection uses `persistedInspectConcurrency`; the synchronous projector must retain only owned output.

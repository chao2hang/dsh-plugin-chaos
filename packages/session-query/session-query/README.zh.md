...
- `projectSessions(sessionIds, project, signal?)` 在每个实时优先的完整日志被借用期间折叠它，并按顺序返回每个会话的结算结果。持久化检查使用 `persistedInspectConcurrency`；同步 projector 只能保留自己拥有的输出。
...
**运行时不变式：** 不发布伴生入口。查询结果是不可变的逐调用投影，其谱系与事件关系在构建时被校验；服务不保留可观察的结果状态。

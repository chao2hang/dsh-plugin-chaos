...
`chaos-bundle/cordis.patch.yml` and the web-app patch mount this package as `chaos-restart`; its `enabled` config defaults to true. The host half needs `processControl` (`@deepseek-ai/dsh-process-control`) — absent it, the routes still answer and report no capability. Removing the row removes both the routes and the settings section.
...
No runtime invariant companion is published; restart state lives in the ProcessControl service and the plugin holds no runtime data of its own.

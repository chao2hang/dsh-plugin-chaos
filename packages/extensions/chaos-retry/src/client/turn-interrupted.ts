...
export const turnInterruptedDefinition: ConversationNodeDefinition<TurnInterruptedData> = {
  ...
  buildViewNode: (context: ConversationNodeContext<TurnInterruptedData>) => {
 ...
 * @param ctx - client context carrying the conversation-event registry.
 * @returns the registry disposer.
 */
export function registerTurnInterrupted(ctx: Context): () => void {
  return ctx.conversationEvents.register(turnInterruptedDefinition)
}

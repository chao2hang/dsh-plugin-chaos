...
/**
 * The registered dock entry. Reads the owner-share snapshot directly (no
 * store, no subscription): the dispatching skeleton re-renders the entry on
 * every session-store change, so the derivation stays current without effects.
 * @param props - runtime share with the snapshot, the public input actions,
 * and the injected translate seat.
 * @returns the strip, or null when the conversation did not end abnormally or
 * carries no resentable user text.
 */
export function RetryDock({ session, inputActions, t }: RetryDockProps) {
  const end = detectAbnormalEnd(session)
  // Memo keeps the resendable text stable while unrelated snapshot fields
  // change, so the callback below does not churn.
  const retryText = useMemo(() => lastUserTextOf(session.nodes), [session.nodes])

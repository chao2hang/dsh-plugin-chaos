...
import { chatNode, makeSession } from './session-fixtures.client.ts'
...
describe('RetryDock', () => {
  it('renders nothing for a normal ending', () => {
    const { container } = render(
      <RetryDock
        session={makeSession({ chatNodes: [] })}
        inputActions={makeInputActions() as never}
        t={t as never}
      />,
    )
  ...
  it('offers the retry after a restart cut the turn short with no streamed content', () => {
    ...
    render(
      <RetryDock
        session={makeSession({ chatNodes: [chatNode('turn-interrupted', { turn: 1, seq: 10, time: 10 })] })}
        inputActions={inputActions as never}
        t={t as never}
      />,
    )

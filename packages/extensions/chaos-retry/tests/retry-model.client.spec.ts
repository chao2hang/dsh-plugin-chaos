...
import { chatNode, makeSession, userNode } from './session-fixtures.client.ts'
...
describe('detectAbnormalEnd', () => {
  ...
  it('stays silent while the turn runs, streams, or keeps queued or pending work', () => {
    expect(detectAbnormalEnd(makeSession({ running: true }))).toBeNull()
    expect(detectAbnormalEnd(makeSession({ partial: { turn: 1 } as never }))).toBeNull()
    expect(detectAbnormalEnd(makeSession({ queue: [{ id: 'q' }] as never }))).toBeNull()
    expect(detectAbnormalEnd(makeSession({ pending: [{ kind: 'approval' }] as never }))).toBeNull()
    expect(detectAbnormalEnd(makeSession({ removed: true }))).toBeNull()
  })
...
describe('lastUserTextOf', () => {
  ...
  it('answers null without any user text to resend', () => {
    ...
    const imageOnly: ConversationNode = {
      ...
      content: [{ type: 'image', attachment: { attachmentId: 'a1', mediaType: 'image/png', bytes: 4, width: 1, height: 1 } }],
    }

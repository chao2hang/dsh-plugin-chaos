...
function ThinkTagAssistantContent({
  ...
  const codeLabels = useMemo(() => ({ copyLabel: 'Copy', copiedLabel: 'Copied' }), [])
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    ...
    switch (block.kind) {
      case 'text':
        rendered.push(<MarkdownText key={index} text={block.text} streaming={streaming} codeLabels={codeLabels} fileMentions={mentions} />)
        break
      case 'reasoning':

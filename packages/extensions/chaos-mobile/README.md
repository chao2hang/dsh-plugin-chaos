...
- The composer's paperclip opens a three-action chooser: camera capture, image library, and document upload. Every pick enters the conversation's unified attachment intake — images ride the image rail, other files the background upload queue with file cards; the document action keeps a document-only accept so mobile browsers open the file picker instead of the camera/gallery sheet. The composer's own paperclip stays as the desktop entry.
...
No runtime invariant companion is published; ui-layout owns the panel state and its observable transitions, and the browser half holds no cross-plugin mutable state.

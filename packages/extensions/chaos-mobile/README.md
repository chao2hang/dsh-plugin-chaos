...
- The details column becomes a full-screen pushed page below the nav bar; the nav bar's back button, the system back button, and the core details header's close button all close it through one shared history entry the overlay owns. Its open state is plugin-owned and mirrored as `<html data-chaos-details-open>`: the core column solver keeps its 640px center floor, so a narrow viewport can never render the details column open and the frame's `data-details-collapsed` is a dead signal there. The layout store is written alongside for wide runtimes, where the real column restores on re-widening.
...
- CSS targets stable `data-shell-column`, `data-shell-frame`, `data-shell-handle`, and `data-conversation-session-header` anchors, with a structural fallback — the `data-slot` render sites directly inside each column, and html-level `:has()` probes on the collapsed state attributes — for released frontends (0.1.2-rc.1 and older) that omit the `data-shell-*` hooks. No `[class*=]` hashed-class selectors.
...
- **Layout selectors** — the drawer and sheet CSS targets ui-layout's stable data attributes (`data-shell-frame`, `data-shell-column`, `data-sidebar-collapsed`, `data-details-collapsed`); changes to those emitted attributes require a coordinated update. Released frontends that omit `data-shell-frame` / `data-shell-column` are covered by the structural slot fallback and the html-level collapsed-state probes, which rely on those two state attributes staying frame-exclusive. The details sheet and nav-bar back/menu state additionally key off the plugin-owned `<html data-chaos-details-open>` attribute, since below the solver's 640px center floor the core keeps `data-details-collapsed` set even while the pushed page is open.

...
function candidateRows(files: readonly FileEntry[]): readonly AtFileCandidate[] {
  ...
  return files.map((file) => {
    ...
    return {
      ...
      // The alpha.3 menu contract maps candidate icons to the three built-in
      // kinds; the rich per-extension icons are dropped on this migration.
  ...
  const notifyLexicon = (sessionId: SessionId): void => {
    for (const listener of [...(lexiconListeners.get(sessionId) ?? [])]) {
      try {
        ...
        console.error('[chaos-at-file] lexicon listener failed:', error)
      }
    }
  }

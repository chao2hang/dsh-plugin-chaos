...
export function apply(ctx: ClientContext): void {
  ...
  const openRelative = (relative: string): void => {
    ...
    void connection.api.host.openPath({ path: entry.path }).then((response) => {
      if (!response.result.ok) console.error('[chaos-at-file] path open failed:', response.result.error.message)

...
`usage-report.read` 接收查看者的 IANA 时区，通过 `ctx.sessionQuery.projectSessions` 折叠每份可读取的持久化会话日志，并按查看者时区中的响应日期返回已完成提供方请求的 Token 总量。每个稀疏日期都包含已归因的 `request/context` provider/model 路由总量，客户端无需重建日期与模型的关联即可渲染每天的模型堆叠柱；与当天总量的差额保持为未归因。成功报表会在持久化日志 revision 和已提交的完成请求事件集合未变化时保留在宿主内存，避免重复读取完整日志；下一条完成请求或持久化日志变更会重新构建对应查看者报表。该报表在某份日志无法读取时失败而不会省略它；已取消的读取返回 `cancelled`。它不返回成本，因为请求时实际采用的价格没有持久化。
